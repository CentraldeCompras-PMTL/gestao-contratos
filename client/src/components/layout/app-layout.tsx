      ...
      <div className="p-4 border-t border-border/50">
        <div className="flex items-center justify-between bg-muted/50 rounded-xl p-4">
          <div className="flex flex-col truncate">
            <span className="text-sm font-bold text-foreground truncate">{user?.name || "Usuario"}</span>
            <span className="text-xs text-muted-foreground truncate">{user?.email}</span>
            <span className="text-[11px] uppercase tracking-wide text-muted-foreground">{user?.role === "admin" ? "Administrador" : "Operacional"}</span>
            {user?.role !== "admin" && <span className="text-[11px] text-muted-foreground">Escopo por ente</span>}
          </div>
          <Button variant="ghost" size="icon" onClick={() => logout.mutate()} title="Sair" className="hover:text-destructive shrink-0">
            <LogOut size={18} />
          </Button>
        </div>
      </div>
